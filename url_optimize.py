...
# Plotting all runs on the same graph
...
for i, iterations in enumerate(all_iterations):
    x_values = [x + i * increment for x in range(len(iterations))]
    plt.plot(x_values, all_max_fitness[i], linestyle='-', linewidth=0.3, label=f'Run {i+1}')

# Adding labels and title
plt.xlabel('Iterations')
plt.ylabel('Max Fitness')
plt.title(f'Iterations vs Max Fitness for URL Random Selection for Population size of {pop_size} and Number of Generations of {num_gen}')
...
plt.xlabel('Iterations')
plt.ylabel('Max Fitness')
plt.title(f'Iterations vs Max Fitness for URL Random Search for Population size of {pop_size} and Number of Generations of {num_gen}')
